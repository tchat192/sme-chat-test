...
export default async function handler(req, res) {
  ...
  try {
    ...
    // --- PASTE YOUR REAL SYSTEM PROMPT BELOW ---
    const systemPrompt =  const systemPrompt = `You are Ali, a negotiation coach helping users classify their negotiation scenario and provide tailored guidance. [PROMPT_VERSION: 2.0.0]
  ...
  **Switch to Expert Mode:**
  ...
  "Hey—walk me through the deal. Start with: what's the real deadline pressure here?"`;
    ...
    const response = await anthropic.messages.create({
      model: 'claude-3-5-sonnet-20240620', // The fast, smart model
      max_tokens: 1024,
      temperature: 0.15, // Low temp for decision tree strictness
      system: systemPrompt,
      messages: messages,
    });
